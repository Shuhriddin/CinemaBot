...
async def create_movie(description=None, file_id=None, is_series=False, episode_number=None, caption=None):
    async with aiohttp.ClientSession() as session:
        try:
            data = {
            ...
            async with session.post(f"{URL}/movie/", data=data) as response:
                data = await response.json()
                return data.get('id', "Yuklanmadi!")
        except Exception as e:
            print(f"Error in create_movie: {e}")
...
async def add_episode(series_id, file_id, caption):
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(f"{URL}/series/add_episode/", data={'series_id': series_id, 'file_id': file_id, 'caption': caption}) as response:
                if response.status == 201:
                    data = await response.json()
                    return data
                else:
...
async def get_series_episodes(series_id, page=1):
    async with aiohttp.ClientSession() as session:
        try:
            url = f"{URL}/series/{series_id}/episodes/?page={page}"
            async with session.get(url) as response:
                if response.status == 200:
                    return await response.json()
                return None
...
async def get_episode(series_id, episode_number):
    async with aiohttp.ClientSession() as session:
        try:
             url = f"{URL}/series/{series_id}/episode/{episode_number}/"
             async with session.get(url) as response:
                if response.status == 200:
                    return await response.json()
                return None
...
async def navigate_series(series_id, command):
    async with aiohttp.ClientSession() as session:
        try:
             url = f"{URL}/series/{series_id}/nav/{command}/"
             async with session.get(url) as response:
                if response.status == 200:
                    return await response.json()
                return None
